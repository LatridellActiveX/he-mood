...
/**
 * Nitro bundles `@electric-sql/pglite` into `_libs/` but leaves behind the
 * WASM/data files `import.meta.url` resolves next to that chunk. Copy them
 * after the Vercel output is written so `vite preview` can boot PGLite.
 * Deployed Neon never loads this path.
 */
